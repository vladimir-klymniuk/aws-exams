...
output "caller_identity_arn" {
    description = "The AWS ARN associated with calling entity"
    value       =  element(concat(data.aws_caller_identity.this.*.arn, [""]), 0)
}

output "caller_identity_user_id" {
    description = "The unique identifier of the caling entity"
...
output "iam_account_password_policy_expire_passwords" {
    ...
    value       = true
    #value       = element(concat(data.aws_iam_account_password_policy.this.*.expire_passwords, [""]), 0)
}
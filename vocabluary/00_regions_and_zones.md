...
?    - 'Local Zones'        provide you the ability to place resources, such as compute and storage, 
                            in multiple location close to your end users.

?    - 'AWS Outposts'       brings native AWS services, infrastructure, and operating models to virtually 
...
?    - 'Wavelength Zones'   allow developers to build applications that deliver ultra-low latencies to 5G devices 
                            and end users. Wavelength deploys standard AWS compute and storage services to the edge of telecommunication carriers'5G networks.
```

```txt
      AWS operates state-of-the-art, highly available data centers. 
      Although rare, failures can occur that affect the availability of instances that are in the same location. 
...
```
    ...
    When you view your resources, you see only the resources that are tired to the Region that you specified. 
